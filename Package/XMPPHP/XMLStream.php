...
class XMPPHP_XMLStream {
	...
	public function processUntil($event, $timeout=-1) {
		...
		$updated = '';
		while(!$this->disconnected&&$this->until_count[$event_key] < 1&&(time() - $start < $timeout or $timeout == -1)) {
			$this->__process();
		}
	...
	public function endXML($parser, $name) {
		...
		if($this->xml_depth == 1) {
			...
			foreach($this->xpathhandlers as $handler) {
				if (is_array($this->xmlobj) && array_key_exists(2, $this->xmlobj)) {
					...
					if (($nstag[0] == null or $searchxml->ns == $nstag[0])&&($nstag[1] == "*" or $nstag[1] == $searchxml->name)) {
						foreach($handler[0] as $nstag) {
							if ($searchxml !== null&&$searchxml->hasSub($nstag[1], $ns=$nstag[0])) {
			...
			foreach($this->nshandlers as $handler) {
				if($handler[4] != 1&&array_key_exists(2, $this->xmlobj)&& $this->xmlobj[2]->hasSub($handler[0])) {
					$searchxml = $this->xmlobj[2]->sub($handler[0]);
				} elseif(is_array($this->xmlobj)&&array_key_exists(2, $this->xmlobj)) {
					$searchxml = $this->xmlobj[2];
				}
				if($searchxml !== null&&$searchxml->name == $handler[0]&&($searchxml->ns == $handler[1] or (!$handler[1]&&$searchxml->ns == $this->default_ns))) {
		...
		if($this->xml_depth == 0&&!$this->been_reset) {
