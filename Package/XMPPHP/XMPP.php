...
class XMPPHP_XMPP extends XMPPHP_XMLStream {
	...
	public function presence_handler($xml) {
		...
		} elseif(array_key_exists('type', $xml->attrs)&&$xml->attrs['type'] == 'subscribed') {
	...
	protected function features_handler($xml) {
		if($xml->hasSub('starttls')&&$this->use_encryption) {
			$this->send("<starttls xmlns='urn:ietf:params:xml:ns:xmpp-tls'><required /></starttls>");
		} elseif($xml->hasSub('bind')&&$this->authed) {
			$id = $this->getId();
	...
	*/
	protected function vcard_get_handler($xml) {
		$vcard_array = array();
		$vcard = $xml->sub('vcard');
		// go through all of the sub elements&&add them to the vcard array
