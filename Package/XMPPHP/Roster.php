...
class Roster {
	/**
	 * Roster array, handles contacts&&presence.  Indexed by jid.
	 * Contains array with potentially two indexes 'contact'&&'presence'
	 * @var array
	 */
	...
	public function getPresence($jid) {
		...
		if($this->isContact($jid)) {
			...
			foreach($this->roster_array[$jid]['presence'] as $resource => $presence) {
				...
				if ($presence['priority'] > $current['priority']&&(($presence['show'] == "chat" or $presence['show'] == "available") or ($current['show'] != "chat" or $current['show'] != "available"))) {
					$current = $presence;
