<?php
    ...
    setFn('Generate.Key', function ($Call)
    {
        ...
        F::Run('Code.Run.External.Exec', 'Run',
            [
                'Command' => F::Live('openssl genrsa -out $Filename $KeySize',
                [
    ...
    setFn('Generate.Certificate', function ($Call) {
        F::Run('Code.Run.External.Exec', 'Run',
            [
                'Command' => F::Live('openssl x509 -req -in $CSRFilename -CA $CACertificate -CAkey $CAKey \
    ...
    {
        $Expires = null;
        $Result = F::Run('Code.Run.External.Exec', 'Run',
        [
