        ...
        if (F::Dot($Call, 'JS.Compress.UglifyJS.Enabled')) {
            ...
            if (is_resource($UglifyJSCompressor)) {
                ...
                if (($Code = proc_close($UglifyJSCompressor)) == 0) {
                    ...
                    F::Log(
                        'JS Code is compressed by UglifyJS: From ' . $OriginalSize . ' to ' . $CompressedSize . ' bytes (' . round(
                            100 * (1 - $CompressedSize / $OriginalSize)
                        ) . '%)',
                        LOG_INFO,
                        'Performance'
                    );
                    ...
                    F::Log(
                        'Return code for ' . $Call['JS']['Compress']['UglifyJS']['Command'] . ': ' . $Code,
                        LOG_INFO,
                        'Performance'
                    );
