        ...
        if (isset($Call['Current Image']['Resample']) && !empty($Call['Current Image']['Data']))
        {
            ...
            try
            {
                ...
                if (isset($Call['Current Image']['Height']))
                    ...
                    $Call['Current Image']['Height'] = round($OldHeight * ($Call['Current Image']['Width']/$OldWidth));

                if (isset($Call['Current Image']['Width']))
                    ;
                else
                    $Call['Current Image']['Width'] = round($OldWidth * ($Call['Current Image']['Height']/$OldHeight));

                if ($Result = $Imagick->resizeImage($Call['Current Image']['Width'], $Call['Current Image']['Height'], Imagick::FILTER_BESSEL, 0.5))
                    ;
                else
