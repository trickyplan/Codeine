    ...
    setFn('Run', function ($Call) {
        ...
        if ($Keys = F::Dot($Call, 'Behaviours.Delayed.Keys')) {
            ...
            if ($Result === null) {
                ...
                F::Log(
                    'Delayed Result ' . $ResultID . ' is *queued* with priority P' . F::Dot(
                        $Call,
                        'Behaviours.Delayed.Priority'
                    ),
                    LOG_INFO,
                    'Performance'
                );
                ...
                F::Log(
                    'Delayed Result ' . $ResultID . ' is *ready* with priority P' . F::Dot(
                        $Call,
                        'Behaviours.Delayed.Priority'
                    ) . ' and purged',
                    LOG_INFO,
    ...
    setFn('Run from Queue', function ($Call) {
        ...
        if (empty($Envelopes)) {
            ...
            foreach ($Envelopes as $Envelope) {
                ...
                F::Log(
                    'Delayed Result ' . $Envelope['ID'] . ' is *executed* with priority P' . F::Dot(
                        $Call,
                        'Behaviours.Delayed.Priority'
                    ),
                    LOG_INFO,
