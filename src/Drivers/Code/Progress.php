<?php
    ...
    setFn('Start', function ($Call) {
        ...
        $Call['Progress']['Ticks per log'] = round(
            ($Call['Progress']['Max'] - $Call['Progress']['Min']) / $Call['Progress']['Logs']
        );
        return $Call;
    });
            ...
            if (PHP_SAPI == 'cli') {
                F::Log(
                    'Progress: ' . (round(
                            ...
                            $Call['Progress']['Precision']
                        ) * 100) . '% ' . ($Call['Progress']['Now'] . '/' . ($Call['Progress']['Max'] - $Call['Progress']['Min'])),
                    LOG_NOTICE,
                    'Developer'
                );

                F::Log(
                    'Elapsed: ' . round($Call['Progress']['Elapsed'], $Call['Progress']['Precision']) . ' sec',
                    LOG_NOTICE,
                    'Developer'
                );
                F::Log(
                    'Estimated: ' . round($Call['Progress']['Estimated'], $Call['Progress']['Precision']) . ' sec',
                    LOG_NOTICE,
                    'Developer'
                );
            }
    ...
    setFn('Finish', function ($Call) {
        ...
        if (PHP_SAPI == 'cli') {
            F::Log(
                'Elapsed: ' . round($Call['Progress']['Elapsed'], $Call['Progress']['Precision']),
                LOG_NOTICE,
                'Developer'
            );
        }
