<?php
    ...
    setFn('Add', function ($Call) {
        ...
        if (F::Dot($Call, 'Metric.DryRun') === true) {
            ...
            F::Log(function () use ($Call) {
                return 'Metric Event: ' . F::Dot($Call, 'Metric.Event.Type') . ' ' . j(F::Dot($Call, 'Metric.Event'));
    ...
    setFn('Aggregate', function ($Call) {
        ...
        if ($Count > 0) {
            ...
            F::Log(
                'Getting *' . F::Dot($VCall, 'Metric.Aggregate.Batch.Size') . '* elements from *' . $Type . '*',
                LOG_NOTICE
            );
            $Events = F::Run(
                'IO',
            ...
            if (empty($Events)) {
                ...
                foreach ($Call['Aggregate'] as $Row) {
                    ...
                    F::Log(
                        'Event: *' . $Row['Where']['Type'] . '*. Resolution: *' . $Row['Where']['Resolution'] . '*. Value: *' . $VCall['Data']['Value'] . '*',
                        LOG_NOTICE
                    );
