<?php
    ...
    setFn('Authenticate', function ($Call)
    {
        ...
        $Call = F::Hook('beforeAuthenticate', $Call);
            ...
            else
            {
                ...
                {
                    F::Log('Authentication backend went without errors, but there is no logged user.',
                        LOG_ERR);
                ...
                else
                {
                    ...
                    {
                        F::Log('User *'.$Candidate['ID'].'* is authenticated',
                            LOG_NOTICE, ['Session', 'Security']);

                        $Call = F::Hook('afterAuthenticate', $Call); // Deprecate
                        $Call = F::Hook('Authenticate.Success', $Call);
                    }
                    else
                        F::Log('User is not authenticated',
                            LOG_NOTICE, ['Session', 'Security']);
                }
            }
    ...
    setFn('Set.TTL', function ($Call)
    {
        ...
        F::Log('TTL is *'.$Call['TTL'].'*',
                LOG_INFO, ['Session', 'Security']);
