<?php
    ...
    setFn('Redirect', function ($Call) {
        if (is_string($Call['Redirect'])) {
            if (isset($Call['HTTP']['Headers']['Location:'])) {
                F::Log(
                    'Already was redirected to ' . $Call['HTTP']['Headers']['Location:'] . ', skipping redirect to ' . $Call['Redirect'],
                    LOG_INFO
                );
            } else {
                $URL = $Call['Redirect'];
    ...
    setFn('Server', function ($Call) {
        empty($_SERVER) ? F::Log('Empty $_SERVER', LOG_NOTICE) : F::Log(function () use ($Call) {
            return $_SERVER;
        }, LOG_INFO);
    ...
    setFn('Request', function ($Call) {
        ...
        array_walk_recursive($Call['Request'], function ($Item, $Key) {
    ...
    setFn('Request Headers', function ($Call) {
        foreach ($_SERVER as $Key => $Value) {
            if (preg_match('/HTTP_(.*)/', $Key, $Pockets)) {
                $Call['HTTP']['Request']['Headers'][strtr(
                    ucwords(mb_strtolower(strtr($Pockets[1], '_', ' '))),
                    ' ',
                    '-'
                )] = $Value;
            }
