<?php
    ...
    setFn('Open', function ($Call) {
        ...
        if ($Server === null) {
            ...
            if ($Server !== null) {
                F::Log(
                    '[DEPRECATED] "Server" key will be ousted. Use "Mongo.Server" instead',
                    LOG_WARNING,
        ...
        if ($Database === null) {
            ...
            if ($Database !== null) {
                F::Log(
                    '[DEPRECATED] "Database" key will be ousted. Use "Mongo.Database" instead',
                    LOG_WARNING,
                    ['Developer', 'Deprecated']
                );
            }
        ...
        if ($Server !== null) {
            if ($Database !== null) {
                ...
                F::Log(
                    'Connected to *' . $Server . '* with options: ' . j(F::Dot($Call, 'Mongo.Connect')),
                    LOG_INFO,
                    ['Administrator', 'Mongo']
                );
    ...
    setFn('Options', function ($Call) {
        ...
        if (isset($Call['Limit'])) {
            ...
            F::Log(
                'Sliced *' . $Call['Limit']['To'] . '* from *' . $Call['Limit']['From'] . '*',
                LOG_INFO,
                ['Administrator', 'Mongo']
            );
        }
    ...
    setFn('Read', function ($Call) {
        ...
        if (isset($Cursor)) {
            if ($Cursor === null) {
                ...
                F::Log(
                    count($Data) . ' documents loaded from *' . $Call['Scope'] . '*',
                    LOG_INFO,
                    ['Administrator', 'Mongo']
                );
            }
        ...
        try {
            if (isset($Call['Where'])) {
                ...
                } else // Delete Where
                {
                    ...
                    $Result = $Call['Link']->selectCollection($Call['Scope'])->deleteMany(
                        $Call['Where'],
                        $Call['Mongo']['Options']
                    );
    ...
    setFn('Count', function ($Call) {
        ...
        if (isset($Call['Where']) and $Call['Where'] !== null) {
            ...
            if (isset($Call['Distinct']) && $Call['Distinct']) {
                F::Log(
                    'db.*' . $Call['Scope'] . '*.distinct(' . j($Call['Where']) . ')',
                    LOG_INFO,
                    ['Administrator', 'Mongo']
                );
                $Data = $Call['Link']->selectCollection($Call['Scope'])->distinct(
                    $Call['Fields'][0],
                    $Call['Where'],
                    $Call['Mongo']['Options']
                );
                ...
                F::Log(
                    'db.*' . $Call['Scope'] . '*.count(' . j($Call['Where']) . ')',
                    LOG_INFO,
                    ['Administrator', 'Mongo']
                );
