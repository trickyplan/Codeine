    ...
    */
    ...
    setFn('Select Proxy', function ($Call) {
        ...
        if (isset($Call['CURL']['Proxy']['Host']) && !empty($Call['CURL']['Proxy']['Host'])) {
            F::Log(
                'Proxy: ' . $Call['CURL']['Proxy']['Host'] . ':' . $Call['CURL']['Proxy']['Port'] . ' (' . $Call['CURL']['Proxy']['Type'] . ') selected',
                LOG_INFO,
                'Administrator'
            );
        }
    ...
    setFn('Read', function ($Call) {
        ...
        if (is_array($Call['Where']['ID'])) {
            ...
            if (curl_errno($Call['Link'])) {
                F::Log(
                    'CURL GET error: ' . curl_error($Call['Link']) . ' *' . $Call['Where']['ID'] . '*',
                    LOG_NOTICE,
                    'Administrator'
                );
                F::Log(curl_getinfo($Call['Link']), LOG_INFO, 'Administrator');
        ...
        if (is_array($Call['Where']['ID'])) {
            ...
            foreach ($Call['Where']['ID'] as $cIndex => $cID) {
                ...
                if (F::Dot($Call, 'Headers.' . $cIndex)) {
                    $Call['CURL']['Options'][CURLOPT_HTTPHEADER] = F::Merge(
                        $Call['CURL']['Headers'],
                        $Call['Headers'][$cIndex]
                    );
                }
                ...
                if (F::Dot($Call, 'Data.' . $cIndex)) {
                    $Call['CURL']['Options'][CURLOPT_POSTFIELDS] = is_string(
                        $Call['Data'][$cIndex]
                    ) ? $Call['Data'][$cIndex] : http_build_query($Call['Data'][$cIndex]);
                }
            ...
            if (F::Dot($Call, 'Data')) {
                $Call['CURL']['Options'][CURLOPT_POSTFIELDS] = is_string(
                    $Call['Data']
                ) ? $Call['Data'] : http_build_query($Call['Data']);
            }
            ...
            if (curl_errno($Call['Link'])) {
                F::Log(
                    'CURL POST error: ' . curl_error($Call['Link']) . ' *' . $Call['Where']['ID'] . '* ' . PHP_EOL,
                    LOG_NOTICE,
                    'Administrator'
                );
                F::Log($Result, LOG_NOTICE, 'Administrator');
