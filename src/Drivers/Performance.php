<?php
    ...
    setFn('Do', function ($Call) {
        ...
        if (F::$_Performance == 'Request' or (F::Dot(
            ...
            F::Log(
                'Core Storage: ~*' . (round(mb_strlen(j(F::$_Storage)) / 1024)) . 'kb*',
                LOG_NOTICE,
                'Performance'
            );

            $ExcludedFromLimiting = F::Dot($Call, 'Performance.Excluded');
            foreach (F::$_Counters['T'] as $Key => $Value) {
                if (!isset(F::$_Counters['C']['Call:' . $Key])) {
                    F::$_Counters['C']['Call:' . $Key] = 1;
                }
                ...
                $Call['RCalls'] = round(
                    (F::$_Counters['C']['Call:' . $Key] / $Call['Performance']['Summary']['Calls']) * 100,
                    2
                );
                $Call['ATime'] = round($Value);
                $Call['ACalls'] = F::$_Counters['C']['Call:' . $Key];
                ...
                F::Log(
                    '*' . $Key . '* time per call is *' . $Call['TimePerCall'] . '* ms',
                    $Class['TimePerCall'],
                    'Performance',
                    -1
                );
            ...
            arsort(F::$_Counters['C']);
            foreach (F::$_Counters['C'] as $Key => $Value) {
