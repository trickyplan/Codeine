...
if (self::$Collection->Length>0)
    foreach (self::$Collection as $Object)
        {
            $Data = View::Fusion('Objects/'.self::$Name.'/'.self::$Name.'_GroupBy_'.self::$Aspect, $Object, false);
            $Buffered[$Object->Get(self::$Aspect)][md5($Data)] = $Data;
        }
else
    View::Nest(array('Application/'.self::$Name.'/Empty','Application/_Shared/Empty'));

foreach ($Buffered as $Key => $Text)
    $Output.= View::Fusion('Groups/'.self::$Name.'/'.self::$Aspect, $Object, array('<Group/>'=>$Key, '<Text/>'=>implode('', $Text)));
